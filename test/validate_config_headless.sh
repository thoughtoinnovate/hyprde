...
# &> ...: captures all output
WLR_BACKENDS=headless WLR_RENDERER=pixman Hyprland -c "$CONFIG_DIR/hyprland.conf" &> /tmp/hypr_run.log &
HYPR_PID=$!
...
if grep -Ei "error|invalid field|failed to parse" /tmp/hypr_run.log | grep -v "Could not generate config errors"; then
    echo "--------------------------------------------------"
    echo "CRITICAL: Configuration errors detected!"
    grep -Ei "error|invalid field|failed to parse" /tmp/hypr_run.log
    echo "--------------------------------------------------"
    exit 1
else
