...
parse_shortcuts() {
    ...
    while IFS= read -r line; do
        ...
        if [[ $action =~ (layoutmsg, *(move|movewindowto|swapcol|colresize|togglefit|promote|swaptomaster|addmaster|removemaster)) ]]; then
            scroll_layout+="$entry\n"
        elif [[ $action =~ (killactive|fullscreen|exec.*terminal|exec.*fileManager|togglefloating|exec.*hyprlock|pseudo|togglesplit) ]]; then
...
# The main hyprland.conf sources other files, so we need to check multiple locations
...
# Prefer generated config as it contains the actual bind definitions
